...
/**
 * ProfileComponent - A reusable component for displaying and editing user profiles
 * 
 * Props:
 * - showHeader: boolean - Whether to show the header section (default: true)
 * - headerTitle: string - Custom header title (default: "My Profile")
 * - headerSubtitle: string - Custom header subtitle (default: "Personal Information")
 * - showLogoutButton: boolean - Whether to show logout button in header (default: true)
 * - onBack: function - Callback for back button (optional)
 * - showBackButton: boolean - Whether to show back button (default: false)
 * - targetUserId: string - ID of user to show profile for (default: current user)
 * - isReadOnly: boolean - Whether the profile is read-only (default: false)
 * - containerStyle: object - Custom container styles
 * - onProfileUpdate: function - Callback when profile is updated
 */
export default function ProfileComponent({
  ...
  const cleanupMalformedUrl = async () => {
    try {
      if (userProfile?.photo_url && userProfile.photo_url.includes('blob:')) {
        console.log('🧹 ProfileComponent: Cleaning up malformed URL in database');
        console.log('❌ ProfileComponent: Malformed URL:', userProfile.photo_url);
        ...
        if (error) {
          console.error('ProfileComponent: Error cleaning up malformed URL:', error);
        } else {
          console.log('✅ ProfileComponent: Malformed URL cleaned up successfully');
          // Refresh profile data
          await fetchUserProfile();
        }
      }
    } catch (error) {
      console.error('ProfileComponent: Error in cleanupMalformedUrl:', error);
    }
  };

  useEffect(() => {
    if (profileUserId) {
      console.log('ProfileComponent: User available, fetching profile for ID:', profileUserId);
      fetchUserProfile();
    } else {
      console.log('ProfileComponent: No user ID available, waiting...');
    }
  }, [profileUserId]);
  ...
  const fetchUserProfile = async () => {
    try {
      if (!profileUserId) {
        console.error('ProfileComponent: No user ID available');
        setLoading(false);
        return;
      }
      
      setLoading(true);
      console.log('ProfileComponent: Fetching profile for user ID:', profileUserId);
      
      // If fetching own profile, sync emails first
      if (isOwnProfile && user?.email) {
        console.log('ProfileComponent: Syncing emails before fetching own profile...');
        try {
          const syncResult = await syncEmailWithDatabase(user);
          console.log('ProfileComponent: Email sync completed with result:', syncResult);
          
          // If sync was successful, wait a moment for database to update
          if (syncResult) {
            console.log('ProfileComponent: Waiting for database update to propagate...');
            await new Promise(resolve => setTimeout(resolve, 500));
          }
        } catch (syncError) {
          console.warn('ProfileComponent: Email sync failed, continuing with profile fetch:', syncError);
        }
      }
      
      // First try to fetch from user_profiles view with user ID
      console.log('ProfileComponent: Attempting to fetch from user_profiles with ID:', profileUserId);
      ...
      // If that fails and it's own profile, try with email
      if (error && isOwnProfile && user?.email) {
        console.log('ProfileComponent: Failed to fetch with user ID, trying with email:', user.email);
        ...
        if (!emailError && emailData) {
          console.log('ProfileComponent: Found profile with email, ID mismatch detected');
          data = emailData;
          error = null;
        } else {
          console.log('ProfileComponent: Failed to fetch with email as well:', emailError);
        }
      }
      
      if (error) {
        console.log('ProfileComponent: Error fetching from user_profiles view:', error);
        
        // Fallback: try to fetch from users table directly
        console.log('ProfileComponent: Trying fallback to users table with ID:', profileUserId);
        ...
        if (userError) {
          console.error('ProfileComponent: Error fetching from users table:', userError);
          
          // If it's own profile and user doesn't exist, try to create basic profile
          if (isOwnProfile && user?.email) {
            console.log('ProfileComponent: User not found, attempting to create basic profile...');
            ...
            if (createError) {
              console.error('ProfileComponent: Error creating user profile:', createError);
              Alert.alert('Error', 'Failed to create profile. Please contact support.');
              return;
            }
            
            console.log('ProfileComponent: Created basic user profile:', newUserData);
            data = {
              ...newUserData,
          ...
          console.log('ProfileComponent: Successfully fetched from users table:', data);
        }
      } else {
        console.log('ProfileComponent: Successfully fetched from user_profiles view:', data);
      }
      ...
      console.error('ProfileComponent: Error in fetchUserProfile:', error);
      Alert.alert('Error', 'Failed to fetch profile data. Please try again.');
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = () => {
    if (isReadOnly) {
      showToast('This profile is read-only', 'warning');
      return;
    }
    setEditedProfile({ ...userProfile });
    setEditing(true);
  ...
  const handleSave = async () => {
    console.log('🔥 ProfileComponent: handleSave function called');
    console.log('📊 ProfileComponent: Current userProfile:', userProfile);
    console.log('✏️ ProfileComponent: Current editedProfile:', editedProfile);
    ...
    try {
      ...
      console.log('👤 ProfileComponent: Current user role:', currentUserRole);
      ...
      const result = await ProfileEditor.saveProfile({
        ...
        onSuccess: (updatedProfile) => {
          console.log('✅ ProfileComponent: ProfileEditor success callback called');
        ...
        onError: (error) => {
          console.error('❌ ProfileComponent: ProfileEditor error callback:', error);
          ...
          if (error.message && error.message.includes('mobile number')) {
            ...
            Alert.alert('Validation Error', error.message || 'An error occurred while saving the profile.');
          }
        }
      });
      
      console.log('📋 ProfileComponent: ProfileEditor result:', result);
      
      // If save was successful, log the updated fields
      if (result.success) {
        console.log('🎉 ProfileComponent: Updated fields:', result.updatedFields);
      }
      
    } catch (error) {
      console.error('💥 ProfileComponent: Unexpected error in handleSave:', error);
      Alert.alert('Error', 'An unexpected error occurred while saving the profile.');
    } finally {
      console.log('🏁 ProfileComponent: handleSave finally block');
      setSaving(false);
    }
  };
  ...
  const pickImage = async () => {
    try {
      console.log('ProfileComponent: pickImage function called');
      console.log('ProfileComponent: Platform:', Platform.OS);
      
      // For web, use HTML input element
      if (Platform.OS === 'web') {
        console.log('ProfileComponent: Using web file picker');
        ...
        input.onchange = async (event) => {
          ...
          if (file) {
            console.log('ProfileComponent: File selected:', file);
            ...
            console.log('ProfileComponent: File URL created:', fileURL);
      ...
      console.log('ProfileComponent: Using expo-image-picker for mobile');
      ...
      console.log('ProfileComponent: Image picker result:', result);
      ...
      console.error('ProfileComponent: Error picking image:', error);
  ...
  const deletePreviousImageByUrl = async (photoUrl) => {
    try {
      console.log('=== ProfileComponent: DELETE PREVIOUS IMAGE DEBUG ===');
      console.log('🗑️ ProfileComponent: deletePreviousImageByUrl function called');
      console.log('ProfileComponent: photoUrl to delete:', photoUrl);
      
      if (!photoUrl) {
        console.log('❌ ProfileComponent: No previous image URL provided');
        return;
      }
      ...
      console.log('ProfileComponent: URL patterns tried:', urlPatterns.length);
      console.log('ProfileComponent: Matched pattern:', matchedPattern);
      console.log('ProfileComponent: Extracted filename:', fileName);
      
      if (!fileName) {
        console.log('ProfileComponent: Could not extract filename from URL:', photoUrl);
        console.log('ProfileComponent: URL structure not recognized, skipping deletion');
        return;
      }
      
      console.log('🗑️ ProfileComponent: Attempting to delete file:', fileName);
      console.log('🔗 ProfileComponent: From bucket: profile-images');
      ...
      console.log('📊 ProfileComponent: Delete operation result:', { data, error });
      
      if (error) {
        console.error('❌ ProfileComponent: Error deleting previous image:', error);
        // Don't throw error, just log it - we still want to upload the new image
      } else {
        console.log('✅ ProfileComponent: Previous image deleted successfully');
        console.log('📄 ProfileComponent: Delete response data:', data);
      }
    } catch (error) {
      console.error('ProfileComponent: Error in deletePreviousImage:', error);
      // Don't throw error, just log it - we still want to upload the new image
    }
  };
  ...
  const uploadProfileImage = async (imageAsset) => {
    try {
      ...
      console.log('ProfileComponent: Previous photo URL saved for cleanup:', previousPhotoUrl);
      ...
      console.log('📁 ProfileComponent: Generated filename:', fileName);
      console.log('📎 ProfileComponent: File extension:', fileExtension);
      console.log('🔗 ProfileComponent: Original URI:', imageAsset.uri);
      
      console.log('ProfileComponent: Starting upload for file:', fileName);
      console.log('ProfileComponent: Image asset:', imageAsset);
      
      let blob;
      
      // Handle web vs mobile differently
      if (Platform.OS === 'web' && imageAsset.uri.startsWith('blob:')) {
        console.log('ProfileComponent: Processing web blob URL');
        ...
        console.log('ProfileComponent: Processing mobile image URI');
      ...
      console.log('ProfileComponent: Blob created, size:', blob.size, 'type:', blob.type);
      ...
      console.log('ProfileComponent: Upload result:', { data, error });

      if (error) {
        console.error('ProfileComponent: Storage upload error:', error);
        Alert.alert('Upload Failed', `Failed to upload image: ${error.message}`);
        return;
      }

      console.log('ProfileComponent: Upload successful, getting public URL...');
      ...
      console.log('ProfileComponent: Public URL data:', urlData);
      ...
      if (urlData?.publicUrl) {
        ...
        console.log('ProfileComponent: Profile updated with new image URL:', urlData.publicUrl);
        ...
        console.log('🔄 ProfileComponent: Auto-saving photo_url to database...');
        try {
          ...
          if (dbError) {
            console.error('❌ ProfileComponent: Error saving photo_url to database:', dbError);
            ...
            console.log('✅ ProfileComponent: Photo URL saved to database successfully');
          ...
          console.error('💥 ProfileComponent: Unexpected error saving to database:', saveError);
          showToast('Image uploaded but not saved to database. Please click Save Changes.', 'warning');
        }
      } else {
        console.error('ProfileComponent: Failed to get public URL');
        Alert.alert('Upload Error', 'Failed to get image URL. Please try again.');
      }
    } catch (error) {
      console.error('ProfileComponent: Error uploading image:', error);
  ...
  const getProfileImageOrInitials = () => {
    ...
    console.log('=== ProfileComponent: IMAGE LOADING DEBUG ===');
    console.log('ProfileComponent: editedProfile.photo_url:', editedProfile?.photo_url);
    console.log('ProfileComponent: userProfile.photo_url:', userProfile?.photo_url);
    console.log('ProfileComponent: Final imageUrl:', imageUrl);
    console.log('ProfileComponent: imageLoadError:', imageLoadError);
    ...
    if (showInitials) {
      console.log('🔤 ProfileComponent: Showing initials because:', !imageUrl ? 'No image URL' : 'Image load error');
      return (
      ...
      console.log('🖼️ ProfileComponent: Attempting to load image from:', cacheBustingUrl);
      ...
      return (
        <Image 
          ...
          onError={(error) => {
            console.log('❌ ProfileComponent: Profile image failed to load from:', cacheBustingUrl);
            console.log('ProfileComponent: Error details:', error);
            console.log('🔄 ProfileComponent: Setting imageLoadError to true');
            setImageLoadError(true);
          }}
          onLoad={() => {
            console.log('✅ ProfileComponent: Profile image loaded successfully from:', cacheBustingUrl);
            setImageLoadError(false);
          }}
        />
      );
    ...
    return (
      <View style={[styles.errorContainer, containerStyle]}>
        ...
        <Text style={styles.errorSubtext}>
          This might be due to an email mismatch between login and database records. Try "Sync & Retry" to fix this automatically.
        </Text>
        <View style={styles.errorButtonContainer}>
          ...
          {isOwnProfile && (
            <TouchableOpacity 
              ...
              onPress={async () => {
                try {
                  console.log('ProfileComponent: Manual email sync triggered from error screen');
                  await syncEmailWithDatabase(user);
                  setTimeout(() => fetchUserProfile(), 1000);
                } catch (error) {
                  console.error('ProfileComponent: Manual email sync failed:', error);
                  fetchUserProfile();
                }
              }}
            >
  ...
  return (
    <View style={[styles.container, containerStyle]}>
      ...
      >
        ...
        {/* Authentication Section - Only show for own profile */}
        {isOwnProfile && (
      ...
      {/* Authentication Change Modal - Only show for own profile */}
      {isOwnProfile && (
        <AuthChangeModal
