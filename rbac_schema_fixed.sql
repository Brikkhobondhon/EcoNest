-- Fixed Enhanced RBAC Schema for EcoNest
-- This properly handles the migration from old role column to new user_role table
...
-- 3. Add role_id column to users table (keep old role column for now)
ALTER TABLE users ADD COLUMN IF NOT EXISTS role_id UUID REFERENCES user_role(id);

-- 4. Update existing users to have proper role_id based on their current role
UPDATE users 
SET role_id = (SELECT id FROM user_role WHERE role_name = users.role)
WHERE role_id IS NULL;

-- 5. Create user_permissions table for additional user-specific permissions
...
-- 6. Create user_sessions table for session management
...
-- 8. Drop existing policies that depend on the old role column
...
-- 9. Drop and recreate the user_profiles view
...
-- 10. Create new enhanced policies using role_id
...
-- 11. Policies for user_role table
...
-- 12. Policies for user_permissions table
...
-- 13. Policies for user_sessions table
...
-- 15. Create function to get user role
...
-- 16. Create trigger to update user_role timestamp
...
-- 18. Create sample users with different roles
INSERT INTO users (
    user_id, 
    email, 
    password, 
...
-- 19. Now we can safely drop the old role column
ALTER TABLE users DROP COLUMN IF EXISTS role CASCADE;

-- 20. Verify the migration worked
SELECT 'Migration completed successfully. User profiles with roles:' as status;
SELECT user_id, email, role_name, department_name FROM user_profiles; 